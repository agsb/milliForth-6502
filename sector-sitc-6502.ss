...
;
;   only immediate flag used as $80, no hide, no compile, no extfst flags;
;
...
;   For 6502:
;
;   hardware stack not used as forth stack, free for jsr/rts and pha/pla
;
;   6502 is a byte processor, no need 'pad' at end of even names;
; 
;   no multiuser, no multitask, no faster;
;
;   24/01/2024 for comparison with x86 code, rewrite using standart indirect thread code;
;
;----------------------------------------------------------------------
;
; stuff for ca65 
...
@loop:
    ...
    ; get that link, and bypass the link 
    ldx #(fst - nil) ; from 
    ldy #(trd - nil) ; into
    jsr pull
...
;---------------------------------------------------------------------
; all primitives ends with jmp next_
;
spull2:
    ldy #(snd - nil)
    jsr spull
...
storew:
    jsr spull2
    ldx #(snd - nil) ; push starts with decw, then it 
    ldy #(snd - nil)
    jsr push
    jmp next_
